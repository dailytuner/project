...
from ..database.models import NatalChart, UserProfile
...
async def create_and_save_natal_chart(
    ...
    """
    Создание и сохранение натальной карты на основе данных профиля

    Args:
        user_id: ID пользователя
        birth_datetime: Дата и время рождения (локальное)
        timezone: Часовой пояс (если None - берется из профиля)
        house_system: Система домов

    Returns:
        Сохраненная натальная карта
    """
    try:
        logger.info(f"🌟 Создание натальной карты для user_id={user_id}")
        ...
        async with async_session() as session:
            ...
            # 2. Проверяем наличие геоданных
            if not profile.birth_lat or not profile.birth_lng:
                raise ValueError(
                    f"No geodata for user {user_id}. "
                    "Please update profile with city information first."
                )

            # 3. Определяем часовой пояс
            tz = timezone or profile.birth_timezone or 'Europe/Moscow'

            logger.info(
                f"📍 Используем данные профиля: "
                f"{profile.birth_city} ({profile.birth_lat}, {profile.birth_lng})"
            )

            # 4. Получаем калькулятор
            calculator = await get_natal_calculator()

            # 5. Рассчитываем карту с координатами из профиля
            natal_data = await calculator.calculate_natal_chart(
                lat=float(profile.birth_lat),
                lon=float(profile.birth_lng),
                birth_datetime_local=birth_datetime,
                timezone_str=tz,
                city_name=profile.birth_city,
                country_code=profile.birth_country_code,
                region=profile.birth_region,  # ✅ Передаем регион
                house_system=house_system,
                include_all=True,
                include_heavy=True
            )

            # 6. Проверяем статус расчета
            ...
            # 7. Сохраняем карту
            return await _save_natal_chart(
                user_id=user_id,
                profile=profile,
...
async def _save_natal_chart(
    ...
    """
    Сохранение натальной карты в БД
    ...
    async with async_session() as session:
        ...
        # 1. Геоданные из профиля
        chart.city_name = profile.birth_city
        chart.birth_lat = profile.birth_lat
        chart.birth_lng = profile.birth_lng
        chart.birth_timezone = profile.birth_timezone or natal_data.get('birth_timezone', 'Europe/Moscow')
        chart.birth_country_code = profile.birth_country_code or natal_data.get('birth_country_code', 'RU')
        chart.birth_region = profile.birth_region or natal_data.get('birth_region')  # ✅ НОВОЕ поле

        # 2. Информация о геокодинге из профиля
        chart.geocoder_query = profile.geocoder_query
        chart.geocoder_full_name = profile.geocoder_full_name
        chart.geocoder_source = profile.geocoder_source or natal_data.get('geocoder_source', 'nominatim')
        chart.system_language = profile.system_language or natal_data.get('system_language', 'ru')
        ...
        # 4. Основные астрологические данные
        ...
        chart.calculation_metadata = {
            ...
            'profile_city': profile.birth_city,
            'profile_region': profile.birth_region
        }

        # 14. Система домов и статус
    ...
    try:
        async with async_session() as session:
            ...
            result_dict = {
                ...
                'region': chart.birth_region,  # ✅ НОВОЕ поле
                'geocoder_source': chart.geocoder_source,
                'geocoder_query': chart.geocoder_query,  # ✅ НОВОЕ поле
                'geocoder_full_name': chart.geocoder_full_name,  # ✅ НОВОЕ поле
...
async def get_natal_chart_stats() -> Dict[str, Any]:
    ...
    try:
        async with async_session() as session:
            ...
            # ✅ Дополнительная статистика по геокодингу
            geocoder_result = await session.execute(
                select(
                    NatalChart.geocoder_source,
                    func.count(NatalChart.id)
                )
                .group_by(NatalChart.geocoder_source)
            )
            geocoder_sources = {row[0]: row[1] for row in geocoder_result}
            ...
            return {
                ...
                'geocoder_sources': geocoder_sources,  # ✅ НОВОЕ
